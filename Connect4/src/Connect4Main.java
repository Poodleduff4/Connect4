...
//diagonal check
//for (int i = column, int j = row;)

/*
        static void TextGrid(int column) {
            ...
            while (true) {
                //output grid with current values
                for (int i = 0; i < 4; i++) {
                    for (int j = 0; j < 4; j++) {
                        if (j == 3) {
                            System.out.print(grid[i][j]);
                ...
                //place the piece at correct coordinate
                ...
                //grid[x - 1][y - 1] = 1;
                System.out.println(winCheck(grid, x, y, count, player));
                if (winCheck(grid, x, y, count, player)) {
                    break;
                }
                player = (player == 2) ? 1 : 2;
                //player++;
